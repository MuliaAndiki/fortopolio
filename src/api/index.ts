...
export async function saveTokens(tokens: TokenPair): Promise<void> {
  ...
  try {
    ...
    if (IS_API_DEBUG) {
      Logger.info('[Auth] saveTokens: accessToken present, refreshToken present');
      try {
...
async function _doRefreshOnce(): Promise<string | null> {
  ...
  if (IS_API_DEBUG) {
    try {
      ...
      Logger.info('[Auth] doRefreshToken: failed to read refresh token length', err);
    }
  }
  ...
  if (IS_API_DEBUG) {
    try {
      ...
      Logger.info('[Auth] doRefreshToken: failed to read response headers', err);
    }
  }
  ...
  if (!res.ok) {
    ...
    console.error(`[Auth] doRefreshToken: endpoint mengembalikan ${res.status}. Body:`, body);
    return null;
  }

  // ── 2. Parse response ────────────────────────────────────────────────────
  let newTokens: TokenPair | undefined;
  try {
    const json: ApiResponse<{ tokens: TokenPair }> = await res.json();
  ...
  if (!newTokens?.accessToken || !newTokens?.refreshToken) {
    console.error('[Auth] doRefreshToken: response tidak mengandung tokens yang valid:', newTokens);
    return null;
  }
  ...
  try {
    if (IS_API_DEBUG) {
      Logger.info('[ Auth] doRefreshToken: menerima tokens dari endpoint refresh. saving...');
...
async function doRefreshToken(): Promise<string | null> {
  ...
  if (_refreshInFlight) {
    Logger.info('[Auth] doRefreshToken: menunggu refresh yang sudah berjalan...');
...
async function coreFetch<T>(
  ...
  const { method = 'GET', body, headers: extraHeaders = {}, cache = 'no-store', next } = config;
